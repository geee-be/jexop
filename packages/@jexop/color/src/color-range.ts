...
export const colorRangeOperator: Operator<string> = ({ from, to, value }) => {
  ...
  const fromColor: chroma.Color = isString(from) || isNumber(from) ? chroma(from) : chroma('#000000');
  const toColor: chroma.Color = isString(to) || isNumber(to) ? chroma(to) : chroma('#ffffff');
